...
@Controller('api/attach')
export class AttachController {
  private readonly logger = new Logger(AttachController.name);
   ...
   */
  @Get(':fileGrpId')
  async list(@Param('fileGrpId') fileGrpId: string) {
    const items = await this.attachService.list(fileGrpId);
    return { items };
  }
  ...
  async upload(
    ...
    const saved = await this.attachService.saveUploadedFiles(fileGrpId, files);
  ...
  async download(
    ...
    return this.attachService.download(fileGrpId, fileId, res);
  }
  ...
  async remove(@Param('fileGrpId') fileGrpId: string, @Param('fileId') fileId: string) {
    await this.attachService.remove(fileGrpId, fileId);
    return { ok: true };
  }
}
